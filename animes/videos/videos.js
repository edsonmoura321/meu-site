            ...
            if (video.currentTime == video.duration){
                    ...
                    setInterval(function(){
                            ...
                            if(i == 0 && verifique == false){
                                window.location.href = `/animes/videos/?name=${nome}&ep=${parseInt(ep)+1}&id=${id}&api=${api}`
                            }
                            i-= 1
    ...
    if(api == 2){
        ...
        fetch(`https://api-python-anime.herokuapp.com/proximo_ep/${id}/anime/${parseInt(ep) + 1}`).then((res) => res.json()).then((date) =>  {
            ...
            for(var i = 0;i < date.data_prox.title.length; i++){
                    ...
                    epp.innerHTML +=
                    ` <div class="img_title"> <a href="/animes/videos/?name=${nome}&ep=${numero_ep}&api=${api}&id=${id}&title=${tilte_epp}">
